...
def send_checkout_notification(order):
    ...
    cash_transaction = order.cash_transaction
    text = (
        ...
        f'Naqd: {cash_transaction.cash_amount}\n'
        f'Karta: {cash_transaction.card_amount}\n'
        f'Qarz: {cash_transaction.credit_amount}\n'
        f'Hamkor hisobi: {cash_transaction.partner_amount}'
    )
...
def send_delivery_checkout_notification(order):
    if not settings.TELEGRAM_BOT_TOKEN or not settings.TELEGRAM_CHAT_IDS:
        return {
            'sent': 0,
            'failed': 0,
    ...
    cash_transaction = order.cash_transaction
    text = (
        f'🛵 Delivery buyurtma #{order.id} yopildi\n'
        f'Mijoz: {order.customer.full_name}\n'
        f'Manzil: {order.delivery_address}\n'
        f'Kuryer: {order.courier_name} ({order.courier_phone})\n'
        f'Jami: {order.total_price}\n'
        f'To‘lov turi: {order.get_payment_type_display()}\n'
        f'Naqd: {cash_transaction.cash_amount}\n'
        f'Karta: {cash_transaction.card_amount}\n'
    ...
    for chat_id in settings.TELEGRAM_CHAT_IDS:
        try:
            ...
            with urlopen(request, timeout=10) as response:
                telegram_response = json.load(response)
            if not telegram_response.get('ok'):
                raise RuntimeError(telegram_response.get('description', 'Telegram API xatosi.'))
            sent += 1
        except HTTPError as exc:
            try:
                error = json.load(exc).get('description', str(exc))
                ...
                error = str(exc) or type(exc).__name__
            errors.append(error)
            logger.exception('Telegram delivery xabari chat_id=%s uchun yuborilmadi: %s', chat_id, error)
        except Exception as exc:
            error = str(exc) or type(exc).__name__
            errors.append(error)
            logger.exception('Telegram delivery xabari chat_id=%s uchun yuborilmadi: %s', chat_id, error)
